...
var ipgeolocation = 'https://api.ipgeolocation.io/ipgeo?apiKey=1785ed53312f42c7b5ef89f65c3faa1a';
  ...
  (function () {
    $.getJSON(ipgeolocation, function (data) {
      writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, function () {
        ...
        writeLine(["Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>", "Welcome back, <i style='color: #0f0'>" + (data.ip ? data.ip : usernames[Math.floor(Math.random() * usernames.length)]) + '</i>! By the way, nice to see someone from ' + (data.country_name ? data.country_name : 'your country') + ' here!'], 30, 500, function () {
          if (app.skippedIntro) return;
